...
export const pdfMapping: PDFMapping = {
  // Données de base
  'project_title': {
    source: 'projectTitle',
  ...
  'rayon': {
    ...
    defaultValue: '0 m',
    transform: (value: number) => value !== undefined && value !== null ? `${Math.round(value * 1000)} m` : '0 m'
  },
  'prix_min': {
  ...
  'credit_total_utilise': {
    ...
    transform: (details) => {
      if (!details) return 0;
      return (details.credit_foncier?.montant_utilise || 0) + (details.credit_accompagnement?.montant_utilise || 0);
    },
    defaultValue: 0
  },
  ...
  'financement_bancaire_pct': {
    ...
    transform: (details) => {
      if (!details) return 0;
      const credit = (details.credit_foncier?.montant_utilise || 0) + (details.credit_accompagnement?.montant_utilise || 0);
      const fonds = details.fonds_propres?.montant_utilise || 0;
  ...
  'total_arrondissement_transactions': {
    ...
    defaultValue: 0,
    transform: (trendSeries: any[]) => {
      if (!trendSeries || !trendSeries.length) return 0;
  ...
  'dvf_table_html': {
    ...
    defaultValue: '',
    transform: (dvfProperties: any[]) => {
      if (!Array.isArray(dvfProperties)) return '';
  ...
  'distribution_chart_url': {
    ...
    defaultValue: '',
    transform: (distributionSeries: any[]) => {
      console.log('[PDF][DISTRIBUTION] distributionSeries:', JSON.stringify(distributionSeries));
      if (!Array.isArray(distributionSeries) || !distributionSeries.length) {
        console.log('[PDF][DISTRIBUTION] distributionSeries is empty or not an array');
        return '';
      }
      // Patch: parse chaque élément si c'est une string JSON
      const parsedSeries = distributionSeries.map(d => (typeof d === 'string' ? JSON.parse(d) : d));
      const labels = parsedSeries.map(d => d.bin || d.price_bin || d.prixM2);
      const data = parsedSeries.map(d => d.count || d.nombreTransactions);
      console.log('[PDF][DISTRIBUTION] labels:', labels);
      console.log('[PDF][DISTRIBUTION] data:', data);
      const chart = new QuickChart();
      chart.setConfig({
        type: 'bar',
        data: {
      ...
      console.log('[PDF][DISTRIBUTION] chart url:', url);
      return url;
    }
  },
};
...
export function mapProjectToPDFData(project: Project): Record<string, any> {
  const result: Record<string, any> = {};
  console.log('[DEBUG RAYON] Projet complet:', JSON.stringify(project, null, 2));

  for (const [key, mapping] of Object.entries(pdfMapping)) {
